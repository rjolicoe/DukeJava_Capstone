...
public class FirstRatings {
    ...
    public ArrayList<Movie> loadMovies(String filename) {
        ...
        for (CSVRecord rec : parser) {
            Movie m = new Movie(rec.get("id"), rec.get("title"), rec.get("year"), rec.get("genre"), rec.get("director"), rec.get("country"), Integer.parseInt(rec.get("minutes")), rec.get("poster"));
            list.add(m);
        }
        return list;
    }
...
//			System.out.println(m);
    ...
    public int numRatings(EfficientRater r) {
        return r.numRatings();
    }
...
//		String filename = "data/ratings.csv";
        ArrayList<Rater> plainRaterList = loadRaters(filename);
        System.out.println("--------------------------------------------------------------");
        System.out.println(filename.substring(5) + " has " + plainRaterList.size() + " raters");
        String raterID = "2";
        HashMap<String, Integer> map = new HashMap<String, Integer>();
        String movieID = "1798709";
        int raterCount = 0;
        Set<String> set = new HashSet<String>();
        for (Rater r : plainRaterList) {
...
//			System.out.println();
        ...
        while (it.hasNext()) {
            ...
            //         System.out.println("Movie ID " + pair.getValue() + " test " + pair.getKey());
        }
        //   System.out.println("Movie ID " + movieID + " was rated by " + raterCount + " raters");
        System.out.println(set.size() + " movies have been rated by all " + plainRaterList.size() + " raters");
        System.out.println("--------------------------------------------------------------");
    }
}
